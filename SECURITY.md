...
- **IdP pull-pollers** (`OKTA_*` / `ENTRA_*`, off by default) are the other outbound path: they call only your configured IdP (`OKTA_LOG_URL` is `safeBaseUrl()`-validated at startup; Entra uses fixed Microsoft hosts). The Okta SSWS token / Entra client secret live in env, are sent only to those hosts, and are never logged. Pulled records run through the same adapters + `sanitizeUpsert` as the push path.
